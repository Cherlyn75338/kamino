...
fn sqrt_price_to_x64_price(sqrt_price: u128, decimals_a: u8, decimals_b: u8) -> ScopeResult<U192> {
    // BOUNDS CHECK: Prevent extreme decimal differences that make overflow more likely
    const MAX_DECIMAL_DIFF: u8 = 24;
    ...
    if decimal_diff > MAX_DECIMAL_DIFF {
        warn!("sqrt_price_to_x64_price: extreme decimal difference {} exceeds max {}", 
    ...
    // CRITICAL FIX: Check for overflow in release builds
    // The high limb must be zero for the value to fit in U192
    if price_u256.0[3] != 0 {
        warn!("sqrt_price_to_x64_price overflow: sqrt_price={}, decimals_a={}, decimals_b={}, high_limb={}",
              sqrt_price, decimals_a, decimals_b, price_u256.0[3]);
        return Err(ScopeError::MathOverflow);
    }
...
pub fn sqrt_price_to_price(
    ...
    let x64_price = if a_to_b {
        ...
        // FIX: Check for truncation in the inversion
        // If the inverted value doesn't fit in u128, it will be truncated
        if inverted_sqrt_price > U192::from(u128::MAX) {
            warn!("sqrt_price_to_price inversion overflow: sqrt_price={}, inverted exceeds u128", sqrt_price);
            return Err(ScopeError::MathOverflow);
        }
...
pub fn price_of_lamports_to_price_of_tokens(
    ...
    if lamport_exp + token_b_decimals >= token_a_decimals {
        ...
        // FIX: Use checked multiplication to prevent overflow
        let factor = 10_u64
            .checked_pow(adjust_exp.try_into().unwrap())
